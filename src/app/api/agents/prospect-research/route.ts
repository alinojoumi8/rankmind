...
import { ProspectResearchAgent } from "@/lib/agents/prospect-research";
...
export async function POST(req: NextRequest) {
  ...
  const usage = await checkAgentUsage(userId, "prospect-research");
  if (!usage.allowed) {
  ...
  const agent = new ProspectResearchAgent();
