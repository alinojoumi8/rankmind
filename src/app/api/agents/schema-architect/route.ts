...
import { SchemaArchitectAgent } from "@/lib/agents/schema-architect";
...
export async function POST(req: NextRequest) {
  ...
  const usage = await checkAgentUsage(userId, "schema-architect");
  if (!usage.allowed) {
  ...
  const agent = new SchemaArchitectAgent();
